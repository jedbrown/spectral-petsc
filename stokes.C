...
int main(int argc,char **args)
{
  ...
  {
    ierr = PetscPrintf(comm, "Norm of solution %f  norm of forcing %f\n", unorm, u2norm);CHKERRQ(ierr);
  }
...
PetscErrorCode StokesCreate(MPI_Comm comm, Mat *A, Vec *X, StokesCtx **ctx)
{
  ...
  m = productInt(d, c->dims); // number of local nodes
  ...
  { // Create differentiation matrices
    ...
    for (int i=0; i < d; i++) cheb_dim[i] = c->dims[i];
    cheb_dim[d] = d+1;
    for (int i=0; i < d; i++) {
  ...
  { // Setup coordinate mapping
    ...
    for (BlockIt it = BlockIt(d, c->dims); !it.done; it.next()) {
      for (int j=0; j < d; j++) {
        x[it.i*d+j] = cos(it.ind[j] * PETSC_PI / (c->dims[j] - 1));
      }
    }
...
PetscErrorCode StokesProcessOptions(StokesCtx *ctx)
{
  ...
  if (!flag) { ctx->numDims = 2; ctx->dims[0] = 8; ctx->dims[1] = 6; }
  ...
  switch (exact) {
    case 0:
      ...
      opt->boundary    = StokesDirichletZero;
...
PetscErrorCode StokesFunction(SNES snes, Vec xG, Vec rhs, void *ctx)
{
  ...
  { // In `gradu' we have gradients [u_x v_x w_x p_x] [u_y v_y w_y p_y] [u_z v_z w_z p_z]
...
PetscErrorCode StokesSetupDomain(StokesCtx *c)
{
  ...
  PetscInt        d = c->numDims, *dim = c->dims, m = productInt(d, dim);;
  IS              isG, isD;
  ...
  for (BlockIt it = BlockIt(c->numDims, c->dims); !it.done; it.next()) {
    for (int j=0; j < d; j++) {
      if (it.ind[j] == 0) {
        n[j] = -1.0;
